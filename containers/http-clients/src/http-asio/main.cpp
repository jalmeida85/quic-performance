...
class client {
 public:
	client(
		boost::asio::io_service &io_service, boost::asio::ssl::context &context, const std::string &server,
		const std::string &path, const std::string &port) : resolver_(io_service), socket_(io_service, context) {
		...
		resolver_.async_resolve(
			query, boost::bind(
				&client::handle_resolve, this, boost::asio::placeholders::error, boost::asio::placeholders::iterator));
	}
	...
	void handle_resolve(
		...
		if (!err) {
			...
			boost::asio::async_connect(
				socket_.lowest_layer(), endpoint_iterator, boost::bind(
	...
	void handle_connect(const boost::system::error_code &err) {
		if (!err) {
			...
			socket_.async_handshake(
				boost::asio::ssl::stream_base::client, boost::bind(
	...
	void handle_read_headers(const boost::system::error_code &err) {
		if (!err) {
			...
			boost::asio::async_read(
				socket_, response_, boost::asio::transfer_at_least(1), boost::bind(
...
int main(int argc, char *argv[]) {
	try {
		if (argc != 4) {
			std::cout << "Usage: http-asio <server> <path> <port>\n";
			return 1;
		}
